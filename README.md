...
Hey there, Brehana here. I wanted to chime in before my robot takes over to talk about an issue I discovered while working with the Arduino Nicla Vision through OpenMV v4.8.1. OpenMV seems to refuse to allocate tensors for .tflite models quantized some data types, regardless of whether the model fits within the memory constraints or not. Attempting to load a .tflite model quantized to int8 or uint8 results in an error stating it failed to allocate tensors for the model. The error it throws is misleading, as it's the same error it throws for trying to load a model that's too big. I'm not sure if this is an error in OpenMV v4.8.1 or in the firmware of the Nicla Vision itself. You will notice in this repo I included a memory probing script (nicla_contiguous_memory_analyzer.py) that I was using to troubleshoot what I thought was a memory issue. By dumb luck I discovered that it will accept models quantized to float32, which this project ends up using. I haven't found much talk about this online, so I'd love to hear someone else's thoughts on the subject in the Discussions forum for this repo.
